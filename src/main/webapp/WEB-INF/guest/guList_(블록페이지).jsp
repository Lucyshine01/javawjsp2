...
<%-- 		</c:if> --%> 
	<div class="text-center">
		<c:if test="${pag > 1}">
			<a href="${ctp}/guList.gu?pag=1" class="btn btn-primary mr-2">첫페이지</a>
		</c:if>
		
		<c:if test="${curBlock > 0}">
			<a href="${ctp}/guList.gu?pag=${(curBlock-1)*blockSize + 1}" class="btn btn-primary mr-2">이전블록</a>
		</c:if>
		<c:forEach var="i" begin="${curBlock*blockSize + 1}" end="${curBlock*blockSize + blockSize}" varStatus="st">
			<c:if test="${i <= totPage}">
				<c:if test="${i == pag}">
					<a href="${ctp}/guList.gu?pag=${i}" class="btn btn-warning">${i}p</a>
				</c:if>
				<c:if test="${i != pag}">
					<a href="${ctp}/guList.gu?pag=${i}" class="btn btn-primary">${i}p</a>
				</c:if>
			</c:if>
		</c:forEach>
		<c:if test="${curBlock < lastBlock}">
			<a href="${ctp}/guList.gu?pag=${(curBlock+1)*blockSize + 1}" class="btn btn-primary ml-2">다음블록</a>
		</c:if>
		<c:if test="${pag < totPage}">
			<a href="${ctp}/guList.gu?pag=${totPage}" class="btn btn-primary ml-2">마지막페이지</a>
		</c:if>
	</div>
<p><br/></p>
