...
<div class="container">
	<div class="text-center" >
		<button type="button" style="vertical-align: top;" style="display: inline;" onclick="location.href='${ctp}/calendar1.st?yy=${yy-1}&mm=${mm}';" class="btn btn-secondary btn-sm" title="이전년도" >◁◁</button>
		<button type="button" style="vertical-align: top;" onclick="location.href='${ctp}/calendar1.st?yy=${yy}&mm=${mm-1}';" class="btn btn-secondary btn-sm" title="이전월" >◀</button>
  	<font size="4" style="display:inline-block; width: 300px;vertical-align: middle;">${yy}년 ${mm+1}월</font>
		<button type="button" style="vertical-align: top;" onclick="location.href='${ctp}/calendar1.st?yy=${yy}&mm=${mm+1}';" class="btn btn-secondary btn-sm" title="다음월" >▶</button>
		<button type="button" style="vertical-align: top;" onclick="location.href='${ctp}/calendar1.st?yy=${yy+1}&mm=${mm}';" class="btn btn-secondary btn-sm" title="다음년도" >▷▷</button>
  	...
  	<table class="table table-bordered" >
  		...
  		<tr style="height: 75px;">
  			...
  			<c:forEach begin="1" end="${lastDay}" varStatus="st">
  				...
  				<c:if test="${gap % 7 == 0 && gap < 30}"></tr><tr style="height: 75px;"></c:if> <!-- 한주가 꽉차면 줄바꾸기 한다. -->
  				<c:set var="gap" value="${gap+1}" />
  			</c:forEach>
  			<c:if test="${(gap-1) % 7 != 0}">
