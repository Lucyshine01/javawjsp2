...
<head>
  ...
  <script>
  	...
  	function idCheck2() {
			...
			// JSON방식 {키(서버에서 받을 변수명):값(변수에 넣을값)}
			// data : {mid: mid},
			$.ajax({
				// 서버의 전송구간
				...
				// 윈도우에서 생략가능(맥일 경우는 잘 모름)
				// contextType: "application/json",
				// charset : "utf-8"
				
				// 간단한 작업에도 기능으로 넣어줌(함수 등)
				success : function(res) {
					alert("서버에서 아이디검색을 성공적으로 마치고 돌아왔습니다. 검색된 성명은 " + res + "입니다");
					$("#demo").html(res);
				},
				error : function() {
					alert("전송 실패!");
  	...
  	function idCheck3() {
			...
			$.ajax({
				...
				// charset : "utf-8"
				success : function(res) {
					alert("서버에서 데이터검색을 성공적으로 마치고 돌아왔습니다.");
					$("#demo").html(res);
					let str = res.split("/");
					$("#tMid").html(str[0]);
					$("#name").html(str[1]);
